...
async function seedOutreach(tenantId: string, _staffIds: string[]): Promise<SeededOutreach> {
  console.log("  → outreach seeding skipped (post-strip — see task #12 import-outreach-legacy)");
  console.log(`    tenant=${tenantId.slice(0, 8)}…`);
  return { leadIds: [], touchIds: [] };
}
...
async function main() {
  ...
  try {
    ...
    console.log("\n── Step 12: Outreach");
    const { leadIds: outreachLeadIds, touchIds: outreachTouchIds } =
