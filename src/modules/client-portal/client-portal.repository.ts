...
function toEngagement(row: EngagementRow): EngagementRecord {
  return {
    ...
    stage: (row.stage ?? null) as import("./client-portal.types").EngagementStage | null,
    clientTenantId: row.clientTenantId ?? null,
    auditWindowStart: row.auditWindowStart ?? null,
    auditWindowEnd: row.auditWindowEnd ?? null,
    closedReason: row.closedReason ?? null,
    planeProjectId: row.planeProjectId ?? null,
    driveFolderId: row.driveFolderId ?? null,
    discoveryCallId: row.discoveryCallId ?? null,
    discoveryNotes: row.discoveryNotes ?? null,
    qualificationData: (row.qualificationData ?? null) as import("./client-portal.types").QualificationData | null,
