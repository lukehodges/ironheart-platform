// ---------------------------------------------------------------------------
// Barrel re-export - all per-module schema files re-exported from a single
// entry point so that all existing imports from "@/shared/db/schema" continue
// to work without any changes.
// ---------------------------------------------------------------------------

export * from "./schemas/tenant.schema"
export * from "./schemas/auth.schema"
export * from "./schemas/customer.schema"
export * from "./schemas/services.schema"
export * from "./schemas/scheduling.schema"
export * from "./schemas/booking.schema"
export * from "./schemas/notifications.schema"
export * from "./schemas/calendar.schema"
export * from "./schemas/shared.schema"
export * from "./schemas/phase6.schema"
export * from "./schemas/resource-pool.schema"
export * from "./schemas/staff.schema"
export * from "./schemas/ai.schema"
export * from "./schemas/event-framework.schema"
export * from "./schemas/outreach.schema"
export * from "./schemas/pipeline.schema"
export * from "./schemas/product.schema"
export * from "./schemas/client-portal.schema"
export * from "./schemas/audit-workspace.schema"
export * from "./schemas/report-generator.schema"
export * from "./schemas/onboarding-chart.schema"
