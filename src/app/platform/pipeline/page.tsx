    ...
    <Popover align="right" width={180} trigger={
      ...
      {(close) => (
        <>
          ...
          <Link href={`/platform/pipeline/${row.id}`} style={{ textDecoration: "none" }}>
    ...
    <aside key={row.id} className="animate-slide-in-right" style={{
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--ih-line)", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <Link href={`/platform/pipeline/${row.id}`} className="ih-btn ih-btn-quiet ih-btn-icon" style={{ height: 24, width: 24, textDecoration: "none" }} title="Open full page">
      ...
      <div className="scrollbar-thin" style={{ flex: 1, overflowY: "auto", padding: 16 }}>
        ...
        {row.stage === "CLOSED_WON" && row.engagementId && (
          <Link href={`/platform/clients/${row.engagementId}`} style={{ textDecoration: "none" }}>
        ...
        <Link href={`/platform/customers/${row.customer.id}`} style={{ textDecoration: "none", color: "inherit" }}>
