      ...
      {customer.engagementIds.length === 0 ? (
        ...
        <div className="ih-card" style={{ padding: 0 }}>
          {customer.engagementIds.map((id, i) => (
            <Link key={id} href={`/platform/clients/${id}`} style={{
      ...
      {customer.invoiceIds.length === 0 ? (
        ...
        <div className="ih-card" style={{ padding: 0 }}>
          ...
          {customer.invoiceIds.map((id, i) => (
            <Link key={id} href={`/platform/payments/${id}`} style={{
  ...
  if (!customer) {
    return (
      <div style={{ padding: 40, textAlign: "center" }}>
        ...
        <Link href="/platform/customers" className="ih-btn ih-btn-accent ih-btn-sm" style={{ textDecoration: "none" }}>
    ...
    <div style={{ margin: "-24px -24px" }}>
      ...
      <div style={{ padding: "22px 28px 18px", borderBottom: "1px solid var(--ih-line)", background: atRisk ? "rgba(192,57,43,0.025)" : "var(--ih-bg)" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 24 }}>
          ...
          <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
            ...
            <button className="ih-btn ih-btn-accent ih-btn-sm" onClick={() => { setToast({ message: "Engagement draft created", tone: "ok" }); router.push("/platform/clients/new") }}>
              <Icon name="plus" size={11} /> New engagement
            </button>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          setMergeTarget(null)
          router.push(`/platform/customers/${mergeTarget.id}`)
        }}
