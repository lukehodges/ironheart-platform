    ...
    <Popover align="right" width={200} trigger={
      ...
      {(close) => (
        <>
          ...
          <Link href={`/admin/customers/${row.id}`} style={{ textDecoration: "none" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
      {row.engagementIds.map(id => (
        <Link key={id} href={`/admin/clients/${id}`} className="ih-card" style={{ padding: 10, textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {row.bookingIds.map(id => (
        <Link key={id} href={`/admin/bookings/${id}`} className="ih-card" style={{ padding: "8px 10px", textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {row.invoiceIds.map(id => (
        <Link key={id} href={`/admin/payments/${id}`} className="ih-card" style={{ padding: "8px 10px", textDecoration: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <span className="ih-mono" style={{ fontSize: 11, color: "var(--ih-ink)" }}>/{id}</span>
          <Icon name="arrowUpRight" size={11} style={{ color: "var(--ih-ink-40)" }} />
        </Link>
      ))}
    ...
    <aside key={row.id} className="animate-slide-in-right" style={{
      ...
      <div style={{ padding: "14px 16px", borderBottom: "1px solid var(--ih-line)", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <Link href={`/admin/customers/${row.id}`} className="ih-btn ih-btn-quiet ih-btn-icon" style={{ height: 24, width: 24, textDecoration: "none" }} title="Open full page">
      ...
      <div style={{ borderTop: "1px solid var(--ih-line)", padding: 12, display: "flex", flexDirection: "column", gap: 6, background: "var(--ih-surface)" }}>
        <Link href={`/admin/customers/${row.id}`} className="ih-btn ih-btn-accent" style={{ width: "100%", textDecoration: "none", justifyContent: "center" }}>
    ...
    <div style={{ display: "flex", height: "calc(100vh - 120px)", margin: "-24px -24px" }}>
      ...
      <section style={{ flex: 1, minWidth: 0, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid var(--ih-line)" }}>
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
            <div>
              <div className="ih-eyebrow" style={{ marginBottom: 6 }}>Customer book · CRM</div>
            ...
            <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
              ...
              <button className="ih-btn ih-btn-accent ih-btn-sm" onClick={() => { setToast({ message: "New customer · draft started", tone: "ok" }); router.push("/admin/customers/cust-nw") }}>
      ...
      {selected && (
        <PreviewDrawer
          ...
          onCreateEngagement={() => { setToast({ message: `Engagement draft created for ${selected.name}`, tone: "ok" }); router.push("/admin/clients/new") }}
