...
function StatusBadge({ status }: { status: StatusType }) {
  switch (status) {
    case "Available":
      return (
        <span className="inline-flex items-center rounded-md border border-blue-300 px-2 py-0.5 text-xs font-medium text-blue-700 bg-white">
          Available
        </span>
      )
    case "Under Offer":
      return (
        <span className="inline-flex items-center rounded-md border border-amber-400 px-2 py-0.5 text-xs font-medium text-amber-800 bg-amber-100">
          Under Offer
        </span>
      )
    case "Let Agreed":
      return (
        <span className="inline-flex items-center rounded-md border border-emerald-500 px-2 py-0.5 text-xs font-medium text-emerald-800 bg-emerald-100">
          Let Agreed
        </span>
      )
    case "Stc":
      return (
        <span className="inline-flex items-center rounded-md border border-purple-400 px-2 py-0.5 text-xs font-medium text-purple-800 bg-purple-100">
          STC
        </span>
      )
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-screen-2xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          />
          <StatCard
            label="Total Enquiries (MTD)"
            value={87}
            icon={<MessageSquare size={18} />}
            trend="up"
          />
          <StatCard
            label="Viewings Booked (MTD)"
            value={34}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100 bg-gray-50/70">
                  <th className="text-left px-5 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap w-[320px]">
                    Property
                  </th>
                  <th className="text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap">
                    Type
                  </th>
                  <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap">
                    Size (sqft)
                  </th>
                  <th className="text-right px-4 py-3 text-xs font-semibold uppercase tracking-wider text-gray-400 whitespace-nowrap">
                    Asking Price / Rent
                    ...
                    <td className="px-4 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <div
                          ...
                          style={{ width: `${Math.round((listing.enquiries / 12) * 40)}px` }}
                        >
                          <div
                        ...
                        <span className="font-medium text-gray-700 tabular-nums w-4 text-right">
                          {listing.enquiries}
                        </span>
                      </div>
                    </td>
                    ...
                    <div className="flex-1 flex items-center gap-2">
                      ...
                      <div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={[
                            "h-full rounded-full",
                            s === "Available" ? "bg-blue-400" :
                            s === "Under Offer" ? "bg-amber-400" :
                            s === "Let Agreed" ? "bg-emerald-500" :
                            "bg-purple-500",
                          ].join(" ")}
                          style={{ width: `${pct}%` }}
                        />
