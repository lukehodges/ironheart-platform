...
function DaysChip({ days }: { days: number }) {
  ...
  return (
    <div
      className={`inline-flex items-center gap-1 rounded border px-1.5 py-0.5 text-[10px] font-semibold tabular-nums ${style}`}
    >
...
function PropertyTypeBadge({ type }: { type: PropertyType }) {
  return (
    <span
      className={`inline-flex items-center rounded border px-1.5 py-0.5 text-[10px] font-semibold tracking-wide uppercase ${PROPERTY_TYPE_STYLES[type]}`}
    >
      {type}
    </span>
  )
...
function DealCard({ deal }: { deal: Deal }) {
  return (
    <div className="rounded-md border border-border bg-card p-3.5 hover:bg-accent/40 transition-colors cursor-default">
      {/* Address + agent */}
      ...
      <div className="flex items-center gap-1.5 mb-1.5">
        ...
        {deal.dealValueType === "sale" && (
          <span className="text-[10px] font-medium text-muted-foreground uppercase tracking-wide ml-0.5">
            Sale
          </span>
        )}
      </div>
  ...
  return (
    <div className="flex flex-col w-72 shrink-0">
      ...
      <div className="flex-1 rounded-b-lg border border-t-0 border-border bg-background/50 overflow-hidden">
        <ScrollArea className="h-full max-h-[calc(100vh-230px)]">
          <div className="p-2.5 flex flex-col gap-2">
            {deals.map((deal) => (
              <DealCard key={deal.id} deal={deal} />
            ))}
            {deals.length === 0 && (
              <p className="text-center text-muted-foreground text-xs py-8">No deals</p>
            )}
          </div>
        </ScrollArea>
      </div>
...
export default function DealsPipelinePage() {
  ...
  const activeDeals =
    PIPELINE.instruction.length +
    PIPELINE["under-offer"].length +
    PIPELINE.exchanged.length
              ...
              <div className="flex items-center gap-2 rounded-lg border border-border bg-card px-4 py-2">
                ...
                {Object.entries(AGENT_AVATAR_STYLES).map(([initials, style]) => (
                  <div key={initials} className="flex items-center gap-1.5">
                    <div
                      className={`h-6 w-6 rounded-full flex items-center justify-center text-[10px] font-bold ${style}`}
                    >
                      {initials}
                    </div>
                  </div>
